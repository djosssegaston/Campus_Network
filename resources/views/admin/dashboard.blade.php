...
<div class="p-8 bg-gray-50 min-h-screen">
        ...
        <div class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <div class="text-xs text-gray-500 mt-2">{{ $usersThisMonth }} ce mois</div>
                </div>
            ...
            <a href="{{ route('dashboard') }}" class="text-blue-600 hover:text-blue-700 text-sm mt-4 inline-block">Retour →</a>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <div class="text-3xl font-bold text-gray-900 mt-2">{{ $totalPublications }}</div>
                    <div class="text-xs text-gray-500 mt-2">{{ $publicationsThisMonth }} ce mois</div>
                </div>
            ...
            <a href="{{ route('publications.index') }}" class="text-green-600 hover:text-green-700 text-sm mt-4 inline-block">Retour →</a>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <div class="text-3xl font-bold text-gray-900 mt-2">{{ $totalGroupes }}</div>
            ...
            <a href="{{ route('groupes.index') }}" class="text-purple-600 hover:text-purple-700 text-sm mt-4 inline-block">Retour →</a>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <div class="text-3xl font-bold text-gray-900 mt-2">{{ $totalMessages }}</div>
                    <div class="text-xs text-gray-500 mt-2">Commentaires: {{ $totalComments }}</div>
                </div>
            ...
            <a href="{{ route('admin.messages.index') }}" class="text-yellow-600 hover:text-yellow-700 text-sm mt-4 inline-block">Gérer →</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <a href="{{ route('users.index') }}" class="text-blue-600 hover:text-blue-700 text-sm mt-4 block">Voir tous →</a>
        </div>
            ...
            <div class="space-y-3">
                @forelse($recentPublications as $pub)
                    <div class="pb-3 border-b last:border-b-0">
                        <p class="text-sm font-medium text-gray-900 truncate">{{ Str::limit($pub->contenu, 30) }}</p>
                        <p class="text-xs text-gray-500">par {{ $pub->utilisateur?->nom ?? 'Unknown' }}</p>
                        <p class="text-xs text-gray-400">{{ $pub->created_at?->diffForHumans() ?? 'N/A' }}</p>
                    </div>
                @empty
                    <p class="text-gray-500 text-sm">Aucune publication récente</p>
                @endforelse
            </div>
            <a href="{{ route('publications.index') }}" class="text-green-600 hover:text-green-700 text-sm mt-4 block">Voir tous →</a>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-3">
                @forelse($recentGroupes as $groupe)
                    <div class="pb-3 border-b last:border-b-0">
                        <p class="text-sm font-medium text-gray-900">{{ $groupe->nom }}</p>
                        <p class="text-xs text-gray-500">Membres: {{ $groupe->utilisateurs?->count() ?? 0 }}</p>
                        <p class="text-xs text-gray-400">{{ $groupe->created_at?->diffForHumans() ?? 'N/A' }}</p>
                    </div>
                @empty
                    <p class="text-gray-500 text-sm">Aucun groupe récent</p>
                @endforelse
            </div>
            <a href="{{ route('groupes.index') }}" class="text-purple-600 hover:text-purple-700 text-sm mt-4 block">Voir tous →</a>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $totalShares }}</p>
                </div>
