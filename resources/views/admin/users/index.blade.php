...
<div class="p-8 bg-gray-50 min-h-screen">
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <form method="GET" action="{{ route('users.index') }}" class="flex gap-4">
            <input type="text" name="search" value="{{ $search }}" placeholder="Chercher par nom ou email..." 
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($utilisateurs as $user)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-600">{{ $user->created_at?->format('d/m/Y H:i') ?? 'N/A' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('users.edit', $user) }}" class="text-blue-600 hover:text-blue-900 mr-3">Modifier</a>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $utilisateurs->total() }}</p>
                </div>
            </div>
        </div>
    </div>
