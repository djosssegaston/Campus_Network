...
@section('title', 'Tableau de Bord - Campus Network')
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
            ...
            <div class="flex justify-between items-center h-16">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div class="pl-4 border-l border-gray-200">
                        <a href="{{ route('profile.edit') }}" class="text-sm text-gray-600 hover:text-gray-900 font-medium">Profil</a>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <!-- Welcome Section -->
        <div class="mb-8">
            <h2 class="text-3xl font-bold text-gray-900 mb-2">Bienvenue, {{ auth()->user()->nom ?? 'Utilisateur' }}</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <a href="{{ route('publications.create') }}" class="btn btn-primary w-full justify-center">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                </svg>
                Nouvelle Publication
            </a>
            <a href="{{ route('groupes.index') }}" class="btn btn-secondary w-full justify-center">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('messages.index') }}" class="btn btn-secondary w-full justify-center">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if($roleSlug === 'etudiant')
            @include('dashboard-components.student')
        @elseif($roleSlug === 'moderateur_groupe')
            @include('dashboard-components.group-moderator')
        @elseif($roleSlug === 'admin_groupe')
            @include('dashboard-components.group-admin')
        @elseif($roleSlug === 'moderateur_global')
            @include('dashboard-components.global-moderator')
        @elseif($roleSlug === 'administrateur')
            @include('dashboard-components.administrator')
        @else
