...
<body class="bg-gray-50">
    ...
    <nav class="fixed top-0 w-full bg-white shadow-md z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="space-x-4">
                    @if(Route::has('login'))
                        <a href="{{ route('login') }}" class="px-6 py-2 text-gray-700 hover:text-blue-600 transition">
                            <i class="fas fa-sign-in-alt mr-2"></i>Se connecter
                        </a>
                    @endif
                    @if(Route::has('register'))
                        <a href="{{ route('register') }}" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
    ...
    <section class="pt-32 pb-20 bg-gradient-to-br from-blue-600 via-blue-500 to-purple-600">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
            ...
            <div class="flex gap-4 justify-center">
                @if(Route::has('login'))
                    <a href="{{ route('login') }}" class="px-8 py-4 bg-white text-blue-600 font-bold rounded-lg hover:bg-gray-100 transition shadow-lg">
                        <i class="fas fa-arrow-right mr-2"></i>Se connecter
                    </a>
                @endif
                @if(Route::has('register'))
                    <a href="{{ route('register') }}" class="px-8 py-4 bg-blue-700 text-white font-bold rounded-lg hover:bg-blue-800 transition shadow-lg">
    ...
    <section class="py-20 bg-white">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div class="flex gap-4 justify-center">
                @if(Route::has('login'))
                    <a href="{{ route('login') }}" class="px-8 py-4 border-2 border-blue-600 text-blue-600 font-bold rounded-lg hover:bg-blue-50 transition">
                        <i class="fas fa-sign-in-alt mr-2"></i>Se connecter
                    </a>
                @endif
                @if(Route::has('register'))
                    <a href="{{ route('register') }}" class="px-8 py-4 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition">
                        <i class="fas fa-user-plus mr-2"></i>Créer un compte
                    </a>
                @endif
            </div>
